...
class Biologic(NupylabInstrument):
    ...
    def _initialize_measurement(
        ...
        record_time: float,
        technique: str,
        measurement: Type[Technique],
        **kwargs,
    ) -> None:

        dc_measurements = ["CP", "CA"]
        ac_measurements = ["PEIS", "SPEIS", "GEIS", "SGEIS"]

        freq_steps: int = round((np.log10(max_freq) - np.log10(min_freq)) * ppd) + 1
        technique_dict: dict = globals()[technique + "_DICT"].copy()

        if technique in ac_measurements:
            ...
            elif technique in ("GEIS" or "SGEIS"):
        ...
        self._measurement = measurement(**technique_dict)
    ...
    def set_parameters(
        ...
        **kwargs,
        ...
        """Set measurement parameters and prepare measurement technique.
        ...
        Args:
            ...
            technique: measurement technique to run, must be `PEIS`, `GEIS`, `SPEIS`, or
                `SGEIS`. Defaults to `PEIS`.
            measurement_condition: function indicating whether to begin measurement measurement.
        ...
        if technique not in ("PEIS", "GEIS", "SPEIS", "SGEIS", "CP", "CA"):
            raise KeyError(
                f"Technique {technique} must be `PEIS`, `GEIS`, `SPEIS`, 'CA', 'CP', or `SGEIS`."
            )
        measurement: Type[Technique] = getattr(
            importlib.import_module("nupylab.drivers.biologic"), technique
        )

        self._initialize_measurement(
            initial_step,
            initial_duration,
            maximum_frequency,
            minimum_frequency,
    ...
    def get_data(self) -> List[DataTuple]:
        """Get OCV or measurement data for each channel.
        ...
        with self.lock:
            ...
            if len(self.channels) == 1:
                ...
                self.biologic.load_technique(channel, self._measurement, first=True, last=True)
                self.biologic.start_channel(channel)
            else:
                for c in self.channels:
                    self.biologic.load_technique(c, self._measurement, first=True, last=True)
                self.biologic.start_channels(self._chan_bool)
        ...
        for kbio_data, c in zip(all_data, self.channels):
            ...
            elif "I" in kbio_data.data_field_names: #measuring a CP or CA
                data.append(DataTuple(self.data_label[0], kbio_data.Ewe), DataTuple(self.data_label[1], kbio_data.I))
            else:
                data.append(DataTuple(self.data_label[0], kbio_data.Ewe))
        return data

    @property
    def measurement_condition(self) -> bool:
